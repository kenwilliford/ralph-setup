...
Create `.ralph/launch.sh` — a self-contained launcher so the user doesn't need to copy-paste the loop command:
...
SPEC_TITLE="<spec title>"  # From spec.md

echo "=== Ralph Loop: $SPEC_TITLE ==="
...
if [ -f .ralph/WAITING ]; then
    echo "Loop paused — human action required:"
    echo ""
    cat .ralph/WAITING
    exit 0
fi
...
Make it executable: `chmod +x .ralph/launch.sh`

**Fill in** `<branch-name>` and `<spec title>` from the spec files. The script uses `cd "$(dirname "$0")/.."` so it works from any working directory.

### 8.7 Report Readiness
...
The launch script (created in §8.6) handles stop file checks, status display, and the loop. Users can run it from any directory.
